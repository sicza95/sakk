...
namespace ChessGame
{
    class ChessKnight : ChessPiece
    {
		...
		public override bool MoveSet(int starti, int startj, int endi, int endj, Chessboard currentboard, bool kiiras)
		{
            ...
            for (int i = 0; i < 8; i++)
            {
                if (0 <= knightPositions[i, 0] && knightPositions[i, 0] <= 7 && 0 <= knightPositions[i, 1] && knightPositions[i, 1] <= 7)
                {
                    if (this.Color == "W")
                    {
...
public:
	...
	virtual char GetPiece()
		{
			return 'N';
		}  //AreSquaresLegal here checks:  That the move either moves by one column and two rows or by two columns and one rows.
